...
void nms(std::vector<Detection>& res, float* output, float conf_thresh, float nms_thresh) {
    ...
    for (int i = 0; i < 25200; i++) {
        ...
        // 类别编号
        int max_index = 0;
        // 求将 80个 类别one-hot编码转化为一个0-79的类别编号
        for (int j = 0; j < 80; j++) {
        ...
        if (m.count(max_index) == 0) m.emplace(max_index, std::vector<Detection>());
        m[max_index].push_back(det);
    }
...
std::vector<cv::Mat> process_mask(const float* proto, int proto_size, std::vector<Detection>& dets) {
    ...
    for (size_t i = 0; i < dets.size(); i++) {
        ...
        for (int x = r.x; x < std::min(r.x + r.width, mask_mat.cols); x++) {
            for (int y = r.y; y < std::min(r.y + r.height, mask_mat.rows); y++) {
                float e = 0.0f;
                for (int j = 0; j < 32; j++) {
                    e += dets[i].mask[j] * proto[j * proto_size / 32 + y * mask_mat.cols + x];
                }
                e = 1.0f / (1.0f + expf(-e));
                mask_mat.at<float>(std::max(0,y), std::max(0,x)) = e;
            }
        }
...
void draw_mask_bbox(cv::Mat& img, std::vector<Detection>& dets, std::vector<cv::Mat>& masks, std::unordered_map<int, std::string>& labels_map) {
    ...
    for (size_t i = 0; i < dets.size(); i++) {
        ...
        for (int x = r.x; x < std::min(r.x + r.width, img_mask.cols); x++) {
            for (int y = r.y; y < std::min(r.y + r.height, img_mask.rows); y++) {
                y = std::max(0, y);
                x = std::max(0, x);
